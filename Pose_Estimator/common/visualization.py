...
def render_3d_animation(keypoints, fps, bitrate, azim, output, viewport, limit=-1, size=6):
    """
    TODO
    Render an animation. The supported output modes are:
     -- 'interactive': display an interactive figure
    ...
    lines = []
    points = None
    draw_lines = [[11, 7], [7,14], [7,8], [8,0], [1,0], [4,0], [7,9], [9,10], [11,12],[12,13],[14,15],[15,16],[4,5],[5,6],[1,2],[2,3]]
    def update_video(i):
        nonlocal initialized, image, lines, points
        ...
        if not initialized:
            for n, ax in enumerate(ax_3d):
                pos = poses[n][i]
                for k in range(len(draw_lines)):
                    j = draw_lines[k][0]
                    j_parent = draw_lines[k][1]
                    lines_3d[n].append(ax.plot([pos[j, 0], pos[j_parent, 0]],
                                                [pos[j, 1], pos[j_parent, 1]],
                                                [pos[j, 2], pos[j_parent, 2]], zdir='z', c=col))
            initialized = True
