    ...
    for action in dataset[subject].keys():
        ...
        if 'positions' in anim:
            ...
            for cam in anim['cameras']:
                ...
                pred_3d = np.zeros_like(pos_3d)
                for i in range(pos_3d.shape[0]):
                    for j in range(pos_3d.shape[1]):
                        cur_2d = pos_2d_pixel_space[i,j]
                        ext_2d = np.ones((3))
                        ext_2d[0] = cur_2d[0]/1000
                        ext_2d[1] = cur_2d[1]/1000
                        ext_2d[2] = pos_3d[i,j,2]
                        pred_3d[i,j] = ext_2d #pos_3d[i,j,2] * np.matmul(np.linalg.inv(mat), ext_2d)
                pos_3d = pred_3d
                pos_3d[:, 1:] -= pos_3d[:, :1] # Remove global offset, but keep trajectory in first position
                minimum = min(minimum, np.amin(pred_3d[:,:,2]))
                maximum = max(maximum, np.amax(pred_3d[:,:,2]))
...
def fetch(subjects, action_filter=None, subset=1, parse_3d_poses=True, validation=False):
    out_poses_3d = []
    out_poses_2d = []
    out_camera_params = []
            ...
            if action_filter is not None:
                ...
                for a in action_filter:
                    if action.startswith(a):
                        found = True
                        break
                if validation and not found:
                    continue
                if not validation and found:
                    continue
...
if action_filter is not None:
    print('Selected test actions:', action_filter)

cameras_valid, poses_valid, poses_valid_2d = fetch(subjects_test, action_filter, validation=True)
    ...
    while epoch < args.epochs:
        ...
        if semi_supervised:
            ...
            for _, batch_3d, batch_2d in train_generator.next_epoch():
                inputs_3d = torch.from_numpy(batch_3d.astype('float32'))
                inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
                print("INPUTS 3D SHAPE: " + str(inputs_3d.shape))
                print("INPUTS 3D IS: " + str(batch_3d))
                print("INPUTS 2D SHAPE: " + str(inputs_2d.shape))
                print("INPUTS 2D IS: " +  str(batch_2d))
                if torch.cuda.is_available():
                    inputs_3d = inputs_3d.cuda()
                    inputs_2d = inputs_2d.cuda()
                inputs_3d[:, :, 0] = 0
