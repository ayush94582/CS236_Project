...
from camera import normalize_screen_coordinates, world_to_camera, wrap, project_to_2d, image_coordinates
import copy
...
class DataGenerator(object):
    ...
    def __init__(self, latent_dim):
        ...
        self.pose_dim = 51
        self.fixed_sub = 'S6'
        self.fixed_action = 'Photo'
        self._cameras = copy.deepcopy(h36m_cameras_extrinsic_params)
        self.relevant_joints = [0,1,2,3,6,7,8,12,13,14,15,17,18,19,25,26,27]
        self.cam_intrinsics = np.zeros(9)
        ...
        data = np.load('/nfs/bigbang/student_projects/ayushaga/VideoPose3D/data/data_3d_h36m.npz', allow_pickle=True)['positions_3d'].item()
        self.data = {}
        self.depth_data = {}
        overall_min = 3
        overall_max =  9
        for subject, actions in data.items():
            self.data[subject] = {}
            self.depth_data[subject] = {}
            for action_name, positions in actions.items():
                cam = self._cameras[subject][1]
                self.cam_intrinsics = cam['intrinsic']
                #subsample_lis = 2 * np.arange(int(positions.shape[0]/2))
                #positions = positions[subsample_lis]
                cam_pos = world_to_camera(positions, R=cam['orientation'], t=cam['translation'])
                pos_2d = wrap(project_to_2d, cam_pos, cam['intrinsic'], unsqueeze=True)
                pos_2d_pixel_space = image_coordinates(pos_2d, w=cam['res_w'], h=cam['res_h'])/cam['res_w']
                self.data[subject][action_name] = cam_pos
                cam_pos_z = (np.expand_dims(cam_pos[:,:,2], axis=2) - overall_min)/(overall_max - overall_min)
                self.depth_data[subject][action_name] = np.concatenate((pos_2d_pixel_space, cam_pos_z), axis=2)

    #pos_2d = wrap(project_to_2d, pos_3d, cam['intrinsic'], unsqueeze=True)

    def sample_both_batch(self, batch_type, batch_size, rf):
        latent_sample = np.random.multivariate_normal(np.zeros(self.latent_dim).astype(np.float32), np.eye(self.latent_dim).astype(np.float32), (batch_size, rf))
        ...
        cam_intrins = np.tile(np.expand_dim(self.cam_intrinsics, axis=0), (rf, 1))
        for i in range(batch_size):
            ...
            pose_seq[:,1:,:] = pose_seq[:,1:,:] - np.tile(np.expand_dims(pose_seq[:,0,:], axis=1), (1,16,1))
            real_sample[i,:,:] = np.reshape(pose_seq, (rf, self.pose_dim))
        return latent_sample, real_sample, twod_sample, cam_intrins

    def sample_batch(self, batch_type, batch_size, rf):
        latent_sample = np.random.multivariate_normal(np.zeros(self.latent_dim).astype(np.float32), np.eye(self.latent_dim).astype(np.float32), (batch_size, rf))
        if batch_type == "generator":
            return latent_sample
        lis_keys = [(k,v) for k,v in self.data.items()]
        ...
        for i in range(batch_size):
            ...
            pose_seq = self.data[subject][action][:,self.relevant_joints,:]
            start_ind = np.random.randint(pose_seq.shape[0] - rf)
            real_sample[i,:,:] = np.reshape(pose_seq[start_ind:start_ind+rf, :, :], (rf, self.pose_dim))
        return latent_sample, real_sample

    def sample_depth_batch(self, batch_type, batch_size, rf):
        latent_sample = np.random.multivariate_normal(np.zeros(self.latent_dim).astype(np.float32), np.eye(self.latent_dim).astype(np.float32), (batch_size, rf))
        if batch_type == "generator":
            return latent_sample
        lis_keys = [(k,v) for k,v in self.depth_data.items()]
        subject_inds = np.random.randint(len(lis_keys), size=batch_size)
        real_sample = np.zeros((batch_size, rf, self.pose_dim))
        for i in range(batch_size):
            subject, action = lis_keys[subject_inds[i]]
            ...
            action_ind = np.random.randint(len(action_keys))
            action = action_keys[action_ind]
            pose_seq = self.depth_data[subject][action][:,self.relevant_joints,:]
            start_ind = np.random.randint(max(pose_seq.shape[0] - rf, 0))
            real_sample[i,:,:] = np.reshape(pose_seq[start_ind:min(start_ind+rf, pose_seq.shape[0]-1), :, :], (rf, self.pose_dim))
    ...
    def sample_both_depth_batch(self, batch_type, batch_size, rf):
        latent_sample = np.random.multivariate_normal(np.zeros(self.latent_dim).astype(np.float32), np.eye(self.latent_dim).astype(np.float32), (batch_size, rf))
        if batch_type == "generator":
            return latent_sample
        lis_keys = [(k,v) for k,v in self.depth_data.items()]
        subject_inds = np.random.randint(len(lis_keys), size=batch_size)
        real_sample = np.zeros((batch_size, rf, self.pose_dim))
        real_2d_sample = np.zeros((batch_size, rf, 34))
        for i in range(batch_size):
            subject, action = lis_keys[subject_inds[i]]
            action_keys = [k for k in action.keys()]
            action_ind = np.random.randint(len(action_keys))
            ...
            pose_seq = self.depth_data[subject][action][:,self.relevant_joints,:]
            start_ind = np.random.randint(pose_seq.shape[0] - rf)
            real_2d_sample[i,:,:] = np.reshape(pose_seq[start_ind:start_ind+rf,:,:2], (rf, 34))
            real_sample[i,:,:] = np.reshape(pose_seq[start_ind:start_ind+rf, :, :], (rf, self.pose_dim))
        return latent_sample, real_sample, real_2d_sample

    def sample_normalized_batch(self, batch_type, batch_size, rf):
        latent_sample = np.random.multivariate_normal(np.zeros(self.latent_dim).astype(np.float32), np.eye(self.latent_dim).astype(np.float32), (batch_size, rf))
        if batch_type == "generator":
            return latent_sample
        lis_keys = [(k,v) for k,v in self.data.items()]
        ...
        for i in range(batch_size):
            ...
            start_ind = np.random.randint(pose_seq.shape[0] - rf)
            pose_seq = pose_seq[start_ind:start_ind+rf,:,:]
            pose_seq = pose_seq - np.tile(np.expand_dims(pose_seq[:,0,:], axis=1), (1,17,1))
            real_sample[i,:,:] = np.reshape(pose_seq, (rf, self.pose_dim))
        return latent_sample, real_sample

    def sample_traj_batch(self, batch_type, batch_size, rf):
        latent_sample = np.random.multivariate_normal(np.zeros(self.latent_dim).astype(np.float32), np.eye(self.latent_dim).astype(np.float32), (batch_size, rf))
        if batch_type == "generator":
            return latent_sample
        lis_keys = [(k,v) for k,v in self.data.items()]
